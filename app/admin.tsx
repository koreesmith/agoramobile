...
export default function AdminScreen() {
  ...
  return (
    <Screen>
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
            {['pending', 'actioned', 'dismissed'].map(st => (
              <TouchableOpacity key={st} onPress={() => setReportStatus(st)}
                ...
                <Text style={{ fontSize: 13, fontWeight: '600', color: reportStatus === st ? c.primary : c.textMuted }}>
                ...
                <TouchableOpacity onPress={() => setExpandedReport(isExpanded ? null : r.id)}>
                  <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                    <View style={{ flex: 1 }}>
                      ...
                      {r.rule_text && <Text style={{ fontSize: 12, color: c.textMuted, fontStyle: 'italic', marginTop: 2 }}>Rule: {r.rule_text}</Text>}
                      {r.details ? <Text style={{ fontSize: 15, color: c.textMd, marginTop: 4 }} numberOfLines={isExpanded ? undefined : 2}>{r.details}</Text> : null}
                    </View>
                      ...
                      <>
                        <View style={{ flexDirection: 'row', gap: 8 }}>
                          <TouchableOpacity onPress={() => reviewRep.mutate({ id: r.id, action: 'actioned' })}
                            ...
                            <Text style={{ color: 'white', fontSize: 15, fontWeight: '600' }}>Mark actioned</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={() => reviewRep.mutate({ id: r.id, action: 'dismissed' })}
                            ...
                            <Text style={{ color: c.textMd, fontSize: 15, fontWeight: '600' }}>Dismiss</Text>
                          </TouchableOpacity>
                        </View>
                        ...
                        {r.reported_user_username && !r.is_banned && (
                          <View style={[s.actionSection, { borderColor: c.border }]}>
                            ...
                            <TouchableOpacity
                              ...
                              <Text style={{ color: 'white', fontSize: 15, fontWeight: '600' }}>🚫 Ban user</Text>
                            </TouchableOpacity>
                          </View>
                        )}
                      </>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          ...
          {modLoading ? <Spinner /> : modUsers.length === 0 ? (
            ...
            <View key={u.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 17, fontWeight: '600', color: c.text }}>@{u.username}</Text>
                  ...
                  {u.banned_at && (
                    <View style={[s.violationBadge, { backgroundColor: '#fee2e2', marginTop: 4 }]}>
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#dc2626' }}>BANNED</Text>
                    </View>
                  )}
                  ...
                  {u.is_remote && <Text style={{ fontSize: 12, color: c.textMuted }}>{u.remote_instance}</Text>}
                </View>
                <View style={{ gap: 6 }}>
                  {u.is_suspended && !u.banned_at && (
                    <TouchableOpacity onPress={() => unsuspend.mutate(u.id)}
                      style={[s.smallBtn, { borderColor: c.border, backgroundColor: c.bg }]}>
                      <Text style={{ fontSize: 13, color: c.textMd }}>Unsuspend</Text>
                    </TouchableOpacity>
                  )}
                  {u.banned_at && (
                    <TouchableOpacity onPress={() => unban.mutate(u.id)}
                      style={[s.smallBtn, { borderColor: c.border, backgroundColor: c.bg }]}>
                      <Text style={{ fontSize: 13, color: c.textMd }}>Unban</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>
              ...
              <View key={u.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
                <TouchableOpacity onPress={() => setExpandedUser(isExpanded ? null : u.id)}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 16, fontWeight: '600', color: c.text }}>@{u.username}</Text>
                      <Text style={{ fontSize: 13, color: c.textMuted }}>{u.email} · {u.role}</Text>
                      {u.is_suspended && !u.banned_at && <Text style={{ fontSize: 12, color: '#f59e0b' }}>Suspended</Text>}
                      {u.banned_at && <Text style={{ fontSize: 12, color: '#ef4444' }}>Banned</Text>}
                    </View>
                    <Ionicons name={isExpanded ? 'chevron-up' : 'chevron-down'} size={16} color={c.textMuted} />
                  </View>
                </TouchableOpacity>
                  ...
                  <View style={{ marginTop: 12, gap: 8 }}>
                    ...
                    {u.is_suspended && !u.banned_at && (
                      <TouchableOpacity onPress={() => unsuspendDirect.mutate(u.id)}
                        ...
                        <Text style={{ color: '#92400e', fontSize: 15, fontWeight: '600' }}>Lift suspension</Text>
                      </TouchableOpacity>
                    )}
                    {u.banned_at && (
                      <TouchableOpacity onPress={() => unbanDirect.mutate(u.id)}
                        ...
                        <Text style={{ color: '#dc2626', fontSize: 15, fontWeight: '600' }}>Unban</Text>
                      </TouchableOpacity>
                    )}
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          ...
          {waitlistLoading ? <Spinner /> : waitlistUsers.length === 0 ? (
            ...
            <View key={u.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 17, fontWeight: '600', color: c.text }}>{u.display_name || u.username}</Text>
                  <Text style={{ fontSize: 13, color: c.textMuted }}>@{u.username}</Text>
                  <Text style={{ fontSize: 13, color: c.textMuted }}>{u.email}</Text>
                  {u.created_at && (
                ...
                <View style={{ gap: 6 }}>
                  <TouchableOpacity
                    ...
                    <Text style={{ fontSize: 13, color: '#15803d', fontWeight: '600' }}>Approve</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    ...
                    <Text style={{ fontSize: 13, color: '#dc2626', fontWeight: '600' }}>Reject</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          {settingsLoading ? <Spinner /> : (
            <View style={[s.card, { backgroundColor: c.card, borderColor: c.border, marginBottom: 16 }]}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                <Text style={{ color: c.textMd, fontSize: 13, flex: 1, marginRight: 8 }}>
          ...
          <View style={[s.actionSection, { borderColor: c.border, marginBottom: 16 }]}>
            ...
            <TouchableOpacity
              ...
              <Text style={{ color: 'white', fontSize: 15, fontWeight: '600' }}>Ban instance</Text>
            </TouchableOpacity>
          </View>
          ...
          {instanceBansLoading ? <Spinner /> : instanceBans.length === 0 ? (
            ...
            <View key={ib.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 16, fontWeight: '600', color: c.text }}>{ib.domain}</Text>
                  {ib.reason && <Text style={{ fontSize: 13, color: c.textMuted, marginTop: 2 }}>Reason: {ib.reason}</Text>}
                  {ib.created_at && <Text style={{ fontSize: 12, color: c.textMuted, marginTop: 2 }}>{new Date(ib.created_at).toLocaleDateString()}</Text>}
                </View>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          ...
          <View style={[s.actionSection, { borderColor: c.border, marginBottom: 16 }]}>
            ...
            <TouchableOpacity
              ...
              <Text style={{ color: 'white', fontSize: 15, fontWeight: '600' }}>Block DID</Text>
            </TouchableOpacity>
          </View>
          ...
          {blockedDidsLoading ? <Spinner /> : blockedDids.length === 0 ? (
            ...
            <View key={b.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: '600', color: c.text }}>{b.did}</Text>
                  {b.reason && <Text style={{ fontSize: 13, color: c.textMuted, marginTop: 2 }}>Reason: {b.reason}</Text>}
                </View>
          ...
          <View style={[s.actionSection, { borderColor: c.border, marginTop: 16, marginBottom: 16 }]}>
            ...
            <TouchableOpacity
              ...
              <Text style={{ color: 'white', fontSize: 15, fontWeight: '600' }}>Ban domain</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      )}
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          ...
          {newInviteCreated && (
            <View style={[s.actionSection, { borderColor: '#22c55e', backgroundColor: '#f0fdf4', marginBottom: 12 }]}>
              ...
              <Text style={{ fontFamily: 'monospace', fontSize: 17, color: '#15803d', marginTop: 4 }}>{newInviteCreated}</Text>
              <TouchableOpacity onPress={() => Share.share({ message: newInviteCreated })} style={{ marginTop: 8 }}>
                <Text style={{ color: c.primary, fontSize: 15 }}>Share code →</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {invitesLoading ? <Spinner /> : invites.length === 0 ? (
            ...
            <View key={inv.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontFamily: 'monospace', fontSize: 16, fontWeight: '600', color: c.text }}>{inv.code || inv.token || inv.id}</Text>
                  {inv.created_by_username && <Text style={{ fontSize: 13, color: c.textMuted }}>By @{inv.created_by_username}</Text>}
                  {inv.uses !== undefined && <Text style={{ fontSize: 13, color: c.textMuted }}>Uses: {inv.uses}</Text>}
                  {inv.created_at && <Text style={{ fontSize: 12, color: c.textMuted }}>{new Date(inv.created_at).toLocaleDateString()}</Text>}
                </View>
                <View style={{ flexDirection: 'row', gap: 6 }}>
                  ...
                  <TouchableOpacity onPress={() => Alert.alert('Revoke invite?', undefined, [
                    ...
                    <Text style={{ fontSize: 13, color: c.red }}>Revoke</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          ...
          <View style={[s.actionSection, { borderColor: c.border, marginBottom: 16 }]}>
            ...
            <TouchableOpacity disabled={!newRuleForm.title.trim() || createRule.isPending}
              ...
              <Text style={{ color: 'white', fontSize: 15, fontWeight: '600' }}>Add rule</Text>
            </TouchableOpacity>
          </View>
            ...
            <View key={rule.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              {editingRule?.id === rule.id ? (
                <View style={{ gap: 8 }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 8 }}>
                    ...
                    <TouchableOpacity onPress={() => setEditingRule(null)}
                      ...
                      <Text style={{ color: c.textMd, fontSize: 15 }}>Cancel</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ) : (
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 8 }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 16, fontWeight: '600', color: c.text }}>{idx + 1}. {rule.title}</Text>
                    {rule.description ? <Text style={{ fontSize: 15, color: c.textMd, marginTop: 3 }}>{rule.description}</Text> : null}
                  </View>
                  <View style={{ gap: 4 }}>
                ...
                <View key={entry.id || i} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                    <View style={{ flex: 1 }}>
                      ...
                      {entry.admin_username && <Text style={{ fontSize: 13, color: c.textMuted }}>By @{entry.admin_username}</Text>}
                      {entry.target_username && <Text style={{ fontSize: 13, color: c.textMuted }}>Target: @{entry.target_username}</Text>}
                      {entry.details && <Text style={{ fontSize: 13, color: c.textMd, marginTop: 3 }}>{entry.details}</Text>}
                    </View>
                    {entry.created_at && (
                      <Text style={{ fontSize: 12, color: c.textMuted, flexShrink: 0 }}>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                {auditPage > 0 && (
                  <TouchableOpacity onPress={() => setAuditPage(p => Math.max(0, p - 1))}
                    style={[s.actionBtn, { borderWidth: 1, borderColor: c.border, flex: 1 }]}>
                    <Text style={{ color: c.textMd, fontSize: 15 }}>← Previous</Text>
                  </TouchableOpacity>
                )}
                {auditHasMore && (
                  <TouchableOpacity onPress={() => setAuditPage(p => p + 1)}
                    style={[s.actionBtn, { borderWidth: 1, borderColor: c.border, flex: 1 }]}>
                    <Text style={{ color: c.textMd, fontSize: 15 }}>Next →</Text>
                  </TouchableOpacity>
                )}
              </View>
            </>
      ...
      {tab === 'media' && (
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          <View style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
            ...
            <Text style={{ color: c.textMd, fontSize: 13, marginBottom: 10 }}>
            ...
            <TouchableOpacity
              ...
              <Text style={{ color: c.textMd, fontSize: 15, fontWeight: '600' }}>
            ...
            {orphanScan && (
              <View style={{ marginTop: 14, gap: 10 }}>
                ...
                {orphanScan.count === 0 && (
                  <Text style={{ color: c.textMuted, fontStyle: 'italic', fontSize: 15 }}>No orphaned files found.</Text>
                )}
                  ...
                  <View style={{ borderWidth: 1, borderColor: c.border, borderRadius: 10, overflow: 'hidden' }}>
                    {orphanScan.orphans.map((f, i) => (
                      <View key={f.path} style={{
                        ...
                        <Text style={{ fontSize: 12, color: c.textMuted, flex: 1 }} numberOfLines={1}>{f.path}</Text>
                        <Text style={{ fontSize: 12, color: c.textLight, flexShrink: 0 }}>{(f.size / 1024).toFixed(0)} KB</Text>
                      </View>
                    ))}
                  </View>
                )}
      ...
      {tab === 'pages' && (
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 12 }}>
          <Text style={{ color: c.textMd, fontSize: 13, marginBottom: 12 }}>
            ...
            <View key={p.id} style={[s.card, { backgroundColor: c.card, borderColor: c.border }]}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    <Text style={{ fontSize: 16, fontWeight: '600', color: c.text }}>{p.display_name}</Text>
                    <Text style={{ fontSize: 13, color: c.textMuted }}>@{p.slug}</Text>
                    {p.is_verified && (
                      <View style={[s.violationBadge, { backgroundColor: '#dbeafe' }]}>
                        <Text style={{ fontSize: 11, fontWeight: '700', color: '#2563eb' }}>✓ Verified</Text>
                      </View>
                    )}
                    {p.is_featured && (
                      <View style={[s.violationBadge, { backgroundColor: '#fef3c7' }]}>
                        <Text style={{ fontSize: 11, fontWeight: '700', color: '#92400e' }}>★ Featured</Text>
                      </View>
                    )}
                  </View>
              ...
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                <TouchableOpacity
                  ...
                  <Text style={{ fontSize: 13, fontWeight: '600', color: p.is_verified ? '#2563eb' : c.textMd }}>
                ...
                <TouchableOpacity
                  ...
                  <Text style={{ fontSize: 13, fontWeight: '600', color: p.is_featured ? '#92400e' : c.textMd }}>
...
const s = StyleSheet.create({
  ...
  actionSectionTitle:{ fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
  miniInput:        { borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, fontSize: 15 },
  ...
  statValue:        { fontSize: 27, fontWeight: '700' },
  statLabel:        { fontSize: 15, marginTop: 4 },
})
